...
export class App {
  ...
  public getAppBody(): HTMLElement {
    return this.options.appBody as HTMLElement;
  }
  ...
  public getAppPath(): string {
    return this.options.appPath as string;
  }

  /** Gets the path to the currently displayed page. Returned string does not include application path. */
  public getCurrentPagePath(): string | null {
    const fullPath = window.location.pathname;
    const len = this.options.appPath?.length ?? 0;
